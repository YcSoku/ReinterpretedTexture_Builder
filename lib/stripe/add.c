...
static BOOL new_vertex(double difference, int id1,int id2,
      ...
      else if ((*(vert_norms + last) != id) && ((int)normal_difference != 360))
	{
	  ...
	  if (new_vertex(normal_difference,id,*(vert_norms + last),n))
	      ...
	      else
		{
		  ...
		  (added+last)->normal = realloc((added+last)->normal,
						 sizeof(int) * (added+last)->num);
  ...
  while (flag)
      ...
      if (v2 == temp->edge[0])
	{
	  /*   If greater than 2 polygons adjacent to an edge, then we will
	       only save the first 2 that we found. We will have a small
	       performance hit, but this does not happen often.
	  */
	  if (temp->edge[2] == -1)
	    temp->edge[2] = fnum;
	  else
